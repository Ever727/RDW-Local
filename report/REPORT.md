...
![image-20241221222916315](src/RL.png)
...
我们一共测试了七个环境，每个环境下随机生成了 25 条路径，即随机选取初始点，从初始点出发，每走 2~6m，随机转向一个角度，直到在虚拟环境中走到 400m，每个环境每种路径下我们测试了五种 RDW 算法，分别为 S2C, S2O, APF, ARC, SRL，还有不加 RDW 的对照组 NON
...
![alt text](src/env_1.png)

![alt text](src/env_1_exp.png)

![alt text](src/env_1_result.png)

* 在当前环境下，由于中心有障碍物，S2C, S2O 表现不佳，而 SRL 是在一个空白环境下训得，其行为近似于 S2C 的 'plus' 版，所以以上三种算法趋向于中心，导致多次 reset 出现
* 而 APF 算法可以根据势能调节前进方向，有效地避开了中心的障碍物

#### `env_2`

本环境为人工搭建的障碍物沿中心向四周辐射排列的环境

![alt text](src/env_2.png)

![alt text](src/env_2_exp.png)

![alt text](src/env_2_result.png)

* 当前环境下，S2O 沿中心绕圆的方法易碰到障碍物，导致其表现不佳
* APF 算法依旧可以远离障碍物前进
* 此环境中虚拟世界与物理世界相似，但是由于我们选择了随机的初始点，导致物理和虚拟出现偏差，所以 ARC 方法并未表现出其实际效果

#### `env_3`

本环境是随机生成的规则边界和障碍物环境

![alt text](src/env_3.png)

![alt text](src/env_3_exp.png)

![alt text](src/env_3_result.png)

* 当前随机生成的环境具有一个特点：即其中间空旷，四个角的靠近左右边界处有障碍物堆积，在这种环境下，算法的 reset 次数跟其初始位置有很大关系
* 可以看到 SRL 算法由于其训练出来的趋向中间移动，所以更易跳出边界处的障碍物包围，从而走到空旷的中心地带，效果更佳
* APF 算法如果初始位置在边界旁边，由于其根据势能躲避障碍物的特性，所以会陷入局部最优，在狭小的空间内转向，导致多次 reset，效果不佳

#### `env_4`

本环境是随机生成的规则边界和障碍物环境

![alt text](src/env_4.png)

![alt text](src/env_4_exp.png)

![alt text](src/env_4_result.png)

* 本环境没有特别多需要介绍的

#### `env_5`

本环境是随机生成的不规则边界和不规则障碍物

![alt text](src/env_5.png)

![alt text](src/env_5_exp.png)

![alt text](src/env_5_result.png)

* 当前环境中，由于 APF 只实现了规则边界和规则障碍物的势能计算，所以面对不规则边界和障碍物，APF 无法正确算出势能导致其效果不佳
* 此外由于障碍物集中在环境中心，所以 S2C, S2O, SRL 均表现不佳

#### `env_6`

本环境是随机生成的不规则边界和不规则障碍物

![alt text](src/env_6.png)

![alt text](src/env_6_exp.png)

![alt text](src/env_6_result.png)

* 当前环境与上一个环境类似

#### `env_7`

本环境是生成的虚拟环境和物理环境对称的环境

![alt text](src/env_7.png)

![alt text](src/env_7_exp.png)

![alt text](src/env_7_result.png)

* 当前环境本来是为 ARC 搭建的场景，我们将物理世界和虚拟世界的初始点定成了同一点，但是由于 ARC 策略对于距离差十分敏感，一点的曲率增益就会导致其偏向，进而引发更大的距离差，所以 ARC 表现不佳，需要后续进一步更改环境。

![alt text](src/improve_virt.png)
![alt text](src/improve_ARC.png)
![alt text](src/improve_APF.png)
![alt text](src/improve_SRL.png)
![alt text](src/improve_S2C.png)
![alt text](src/improve_S2O.png)

我们在中心又添加了一个矩形，从而缩短了边界距离，此时 ARC 策略认为虚拟和物理环境差别不大，因此效果远优于其他策略。但可以知道的是，环境带来的细微误差会导致 ARC 策略的不稳定性。我们在测试过程中发现，即使曲率增益的半径设为一个非常大的数，也很难避免物理环境中的偏向，从而导致在中间没有障碍物时，虚拟和物理环境中的用户不会同时到达中心空旷处，进而造成很大的 $\text{dist}(q_t^{phys}, q_t^{virt})$ 差距，从而应用增益，进而会导致进一步的环境不对齐，从而 ARC 策略变差。我们的设想改进方案是，在用户的物理、虚拟环境中多探测几个方向，并且每次随机取用几个方向作为对齐度测算，或是扩大用户的范围，选择在这个更大范围内取到的 $\text{dist}(q_t^{phys}, q_t^{virt})$ 最小值，从而减小 ARC 策略的敏感度。
...
- ARC 策略在环境复杂时效果会好一些，因为它也考虑到了局部最优策略，并且环境越复杂，其对齐程度越可能高，因此 ARC 策略的效果会更好。但如果环境比较简单，则 ARC 策略就会出问题。考虑虚拟与现实环境都十分空旷，此时其他策略都不会失效，但如果 ARC 的虚拟和现实位置不同步，则 ARC 会不断应用增益进行转向，并且也很难调整到对齐位置，于是会造成很多 reset。实际情况中，ARC 很难让用户保持对齐状态，并且在角落时会频繁 reset，并且调整后由于用户短距离平移，通常又会造成进一步 reset，因此效果不佳。而且对齐情况其实非常依赖角度，如果有一点偏差，就会造成很大的对齐区别，因此 ARC 策略在实际应用中也很难达到理想效果。

![alt text](src/ARC_fail.png)
